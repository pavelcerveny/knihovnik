...
export async function fetchAuthors() {
   const db = await Database.load(DB_PATH);
    const authors = await db.select<Author[]>("SELECT * FROM authors ORDER BY name") ?? [];
    return {authors};
}
   ...
   const db = await Database.load(DB_PATH);
    ...
    const books = await db.select<Record<string, unknown>[]>(`
      ...
      const hydratedBooks = books.map((book) => ({
        ...
        category: book.categoryId ? {id: book.categoryId, name: book.categoryName} : null,
        location: book.locationId ? {id: book.locationId, name: book.locationName} : null,
      }))
...
export async function saveBook(book: SaveBook) {
    ...
    let categoryId = null;
    let locationId = null;
    const authorId = book.author?.id ? book.author?.id : (await db.execute(`INSERT INTO authors (name) VALUES (?)`, [book.author.name])).lastInsertId;

    if (book.category.name) {
      categoryId = book.category?.id ? book.category?.id : (await db.execute(`INSERT INTO categories (name) VALUES (?)`, [book.category.name])).lastInsertId; 
    }

    if (book.location.name) {
      locationId = book.location?.id ? book.location?.id : (await db.execute(`INSERT INTO locations (name) VALUES (?)`, [book.location.name])).lastInsertId;
    }

    await db.execute(`INSERT INTO books (name, publish_year, number_of_pages, author_id, category_id, location_id) VALUES (?, ?, ?, ?, ?, ?)`, [book.name, book.publish_year, book.number_of_pages, authorId, categoryId, locationId]);
}
